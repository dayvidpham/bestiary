...
func TestSeriesAll_CensusExact(t *testing.T) {
	const (
		...
		wantVersionLines  = 207 // lines with a non-empty generation (207 -> 211 at the 2026-07-23 refresh; 211 -> 209 as deepseek gen-1 / gen-2 retire in the v0.2.8 slice; UNCHANGED by the free demotion — every versioned line it touches keeps other entities; 209 -> 210 as the cogito decomposition mints the cogito gen-2.1 line; 210 -> 209 as the cogito variant pin then empties the phantom cogito gen-1 line by merging it onto that new line; 209 -> 207 as the gpt tier re-key retires the three gpt-<tier>@5.6 lines and mints the single gpt gen-5.6 line: -3 +1 = -2)
		wantBareLines     = 203 // lines whose entities carry no identity version (204 -> 208 at the 2026-07-23 refresh; UNCHANGED by the v0.2.8 slice; 208 -> 206 as the free demotion empties the deepseek-flash and minimax-m3 lines; 206 -> 208 as the ling/inkling/kling split adds the bare inkling and kling lines). 208 -> 207 as the mimo normalization empties the bare mimo line; 207 -> 206 as the cogito decomposition moves its sole bare occupant onto the new gen-2.1 line; UNCHANGED by the cogito variant pin, which moves only versioned lines. 206 -> 203 as the gpt tier re-key retires the three bare gpt-luna/gpt-sol/gpt-terra lines; their undated successors are variants of gpt and join the bare gpt line, which already existed, so nothing is added. 207 + 203 = 410.

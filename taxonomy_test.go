...
func TestSeriesAll_CensusExact(t *testing.T) {
	const (
		wantSeries        = 411 // 411 -> 419: 2026-07-23 refresh (+4 versioned incl. gemini-3.6, +4 bare); 419 -> 417: v0.2.8 slice — the deepseek dot-lost merges retire the two phantom versioned lines deepseek gen-1 / gen-2 (command/a{translate} joins the existing command/a line, adding none); 417 -> 415: the global free demotion empties the deepseek-flash and minimax-m3 bare lines; 415 -> 417: the ling/inkling/kling split adds the bare `inkling` and `kling` lines (the kling-v2 versioned line is replaced one-for-one by kling@2.6); 417 -> 416: the keyspace-wide mimo normalization empties the bare `mimo` line (all six of its keys move onto the two existing versioned mimo lines); 416 -> 415: the cogito variant pin retires the phantom cogito gen-1 line by merging its sole occupant onto the repaired gen-2.1 line; 415 -> 410: the gpt tier re-key retires six lines and adds one — the three bare gpt-luna/gpt-sol/gpt-terra lines and the three versioned gpt-<tier>@5.6 lines all disappear as the tiers become variants of gpt, the tiers' undated keys join the EXISTING bare gpt line (adding no line), and the dated keys mint the one new gpt gen-5.6 line. -6 +1 = -5; 410 -> 411: the redundant leading-token strip retires NO line and adds exactly one, the agi gen-01 line, because every other key it re-dates lands on a generation line that already existed
		wantVersionLines  = 208 // lines with a non-empty generation (207 -> 211 at the 2026-07-23 refresh; 211 -> 209 as deepseek gen-1 / gen-2 retire in the v0.2.8 slice; UNCHANGED by the free demotion — every versioned line it touches keeps other entities; 209 -> 210 as the cogito decomposition mints the cogito gen-2.1 line; 210 -> 209 as the cogito variant pin then empties the phantom cogito gen-1 line by merging it onto that new line; 209 -> 207 as the gpt tier re-key retires the three gpt-<tier>@5.6 lines and mints the single gpt gen-5.6 line: -3 +1 = -2; 207 -> 208 as the leading-token strip mints the agi gen-01 line)
		wantBareLines     = 203 // lines whose entities carry no identity version (204 -> 208 at the 2026-07-23 refresh; UNCHANGED by the v0.2.8 slice; 208 -> 206 as the free demotion empties the deepseek-flash and minimax-m3 lines; 206 -> 208 as the ling/inkling/kling split adds the bare inkling and kling lines). 208 -> 207 as the mimo normalization empties the bare mimo line; 207 -> 206 as the cogito decomposition moves its sole bare occupant onto the new gen-2.1 line; UNCHANGED by the cogito variant pin, which moves only versioned lines. 206 -> 203 as the gpt tier re-key retires the three bare gpt-luna/gpt-sol/gpt-terra lines; their undated successors are variants of gpt and join the bare gpt line, which already existed, so nothing is added. UNCHANGED by the leading-token strip: the bare agi line keeps agi/mini and agi/pro after agi itself becomes agi@01, and no other bare line empties. 208 + 203 = 411.

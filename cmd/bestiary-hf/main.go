...
//     3a. ENRICH each linked repo with an archive.org snapshot of its live page, asked
//     of the Wayback AVAILABILITY API (read-only; never Save Page Now) through the
//     SAME polite client, so the ≥1 s cadence holds across both hosts and no new
//     backoff is introduced. Best-effort throughout: the documented miss shape
//     {"archived_snapshots":{}}, any final non-2xx (a rate-limiting 429 included) and
//     an unparseable answer all mean "no snapshot recorded this run" — never an
//     error, never a skipped repo.
...
const (
	...
	// waybackAvailableBase is the Internet Archive Wayback AVAILABILITY API: a
	// read-only, unauthenticated lookup of the closest existing snapshot of a URL. It
	// is deliberately NOT Save Page Now — this tool never asks the archive to CREATE a
	// capture (that is a credentialed write path, and every bot in this project is a
	// strictly read-only polite visitor). If a page is genuinely unarchived, a curator
	// captures it by hand.
	waybackAvailableBase = "https://archive.org/wayback/available?url="
)
...
type joinResult struct {
	...
	// ArchivedURL is the Wayback snapshot of this repo's live page, "" when the
	// best-effort lookup recorded none this run. It is attached by the live run, not
	// by joinHF: the join is a pure, offline, catalog-only computation and must stay
	// testable without any network concept at all.
	ArchivedURL string
}
...
// hfNomenOut mirrors one huggingface_nomina.json nomina[] entry. ArchivedURL is
// omitempty: a row with no recorded snapshot writes no key at all, which keeps the
// committed seed byte-identical to its pre-enrichment form for every repo the
// Availability API has never captured (the common case).
...
const hfFileComment = "HARVESTED HuggingFace Hub naming seed (v0.2.8, GH#25/R1). Each entry records a Hub org/repo path an open-weight entity's WEIGHTS live at, as OBSERVED by the offline cmd/bestiary-hf bot fetching the Hub API (https://huggingface.co/api/models). This is the harvested-layer twin of nomen_claims.json's curated claims, with two deliberate differences: (1) NO archive-snapshot fence ON source_url — source_url is the LIVE repo URL (https://huggingface.co/<org>/<repo>), because a harvested nomen cites the live observation the bot made, not durable third-party evidence (the archive-pin policy binds the curated layer ONLY). A live page can still rot, so the OPTIONAL archived_url field records an archive.org snapshot OF source_url alongside it, looked up best-effort from the Wayback Availability API through the same polite client: absent means no snapshot was recorded (never an error), and a present value must satisfy the same shared archive-snapshot shape check the curated fence uses. A miss never erases a snapshot an earlier run found; (2) CASE IS PRESERVED — an HF id is org/repo, 1:1, case-significant (meta-llama/Llama-3.3-70B-Instruct is not .../llama-3.3-70b-instruct), so value and source_url are stored verbatim and the loader cross-checks source_url == 'https://huggingface.co/' + value. Each nomen mints a NomenSchemeHuggingFace naming with attestation {SourceURL: live repo URL, Source: huggingface, Authority: primary (the Hub is authoritative for the huggingface scheme), Method: harvested}. FIELD OWNERSHIP: the bot owns the repo set (fetch-owned, refreshed each run); curation owns any hand override (preserved on refresh). An entity carrying an HF nomen DUAL-attests {models.dev, huggingface}. Graceful-degrade at runtime, LOUD at codegen (ValidateHFNomina). Sorted deterministically; regenerated by cmd/bestiary-hf, never hand-edited except curated overrides."
...
// PRESERVED (merge-on-refresh, the ollama precedent). archived_url is fetch-owned
// with one deliberate exception: a MISS never erases a snapshot an earlier run
// recorded (see the merge note in the body). Unlinked repos never enter the seed
// (they go to the report). Returns the merged document and the sorted unlinked-repo
// list.
func buildHFOutput(joined []joinResult, existing hfFileOut) (hfFileOut, []string) {
	refreshed := map[string]hfNomenOut{}
	var unlinked []string
	// The snapshots the committed seed already carries, so a miss this run can fall
	// back to them rather than erasing them (see the merge note below).
	existingArchived := map[string]string{}
	for _, n := range existing.Nomina {
		if n.ArchivedURL != "" {
			existingArchived[n.Value] = n.ArchivedURL
		}
	}
	for _, j := range joined {
		...
		refreshed[j.Value()] = hfNomenOut{
			...
			ArchivedURL: j.ArchivedURL,
		}
	}

	// archived_url is fetch-owned but a MISS IS NOT A DELETION. A snapshot, once
	// found, is a durable fact about the past: the archive does not un-capture a page.
	// A "" from this run therefore means "no snapshot recorded THIS run" — a rate
	// limit, an outage, a lookup that was never issued — and must never overwrite a
	// snapshot an earlier run already established, which would silently destroy
	// evidence on any throttled refresh. A NEW snapshot does replace an old one (the
	// archive returns the closest capture, and a fresher one is strictly better
	// evidence of what the page said).
	for value, n := range refreshed {
		if n.ArchivedURL != "" {
			continue
		}
		if prev, ok := existingArchived[value]; ok {
			n.ArchivedURL = prev
			refreshed[value] = n
		}
	}
...
// --------------------------------------------------------------------------
// Wayback archive-snapshot enrichment (best-effort, miss-tolerant)
// --------------------------------------------------------------------------

// waybackSnapshot is the "closest" object the Availability API returns when a
// snapshot exists. Only URL is read: the tool records the snapshot address, and the
// capture timestamp is already embedded in that address by construction.
...
// waybackResp is the Availability API envelope. THE MISS SHAPE IS
// {"archived_snapshots":{}} — an empty object, with HTTP 200 and no error and no
// 404. "No Closest key" is therefore a normal, expected outcome and the single
// easiest detail to get wrong; modelling Closest as a pointer makes the absence
// explicit at the type level rather than hiding it in a zero value.
//
// The API returns at most ONE snapshot, not a list. This type deliberately does not
// model a collection: IA's docs note more may be added later, and a client that
// pretended to handle a list today would be untested speculation.
...
// lookupArchivedURL asks the Wayback Availability API for the closest existing
// snapshot of liveURL and returns it, or "" for ANY outcome that is not a usable
// snapshot. It NEVER returns an error, by design: this is best-effort enrichment of
// a nomen that is already complete without it, so a lookup problem must degrade to
// "no snapshot recorded this run" and never fail, skip or reorder a harvest.
//
// Every miss path collapses to "":
//   - the documented miss shape {"archived_snapshots":{}} (HTTP 200, no closest);
//   - a closest object marked available:false, or carrying an empty url;
//   - any final non-2xx — including a 429 that survived hfDoer's Retry-After retry
//     budget, and a 5xx. The archive rate-limits, and a rate-limited run simply
//     records no snapshot and tries again at the next refresh;
//   - a transport error or an unparseable body;
//   - a snapshot URL that does not satisfy bestiary.IsArchiveSnapshotURL, the SAME
//     shared shape check the curated archive fence applies. The API answers with an
//     http:// address, so the scheme is normalized to https:// first (the archive
//     serves both, and the canonical shape the rest of the codebase stores is
//     https). Anything still non-conforming is discarded rather than written: an
//     unusable citation recorded as durable evidence is worse than none.
//
// It issues its request through the SAME politebot.Client the Hub crawl uses, so the
// ≥1 s cadence is enforced ACROSS BOTH HOSTS by one seam (a second client would
// double the effective outbound rate), and it inherits hfDoer's existing
// Retry-After handling unchanged — no new backoff is added here.
func lookupArchivedURL(ctx context.Context, c *politebot.Client, liveURL string) string {
	body, err := c.Get(ctx, waybackAvailableBase+url.QueryEscape(liveURL), jsonAccept)
	if err != nil {
		// Transport failure or a final non-2xx (politebot rejects those): a miss.
		return ""
	}
	var resp waybackResp
	if uerr := json.Unmarshal(body, &resp); uerr != nil {
		return ""
	}
	closest := resp.ArchivedSnapshots.Closest
	if closest == nil || !closest.Available || closest.URL == "" {
		return ""
	}
	snapshot := normalizeArchiveScheme(closest.URL)
	if !bestiary.IsArchiveSnapshotURL(snapshot) {
		return ""
	}
	return snapshot
}

// normalizeArchiveScheme upgrades the http:// web.archive.org prefix the Availability
// API answers with to the https:// form the curated archive policy (and therefore
// bestiary.IsArchiveSnapshotURL) requires. Only the LEADING scheme of the archive
// host is touched — the original URL embedded in the snapshot's tail keeps whatever
// scheme it was captured under, which is why the shared shape check permits http or
// https there.
func normalizeArchiveScheme(u string) string {
	const insecure = "http://web.archive.org/"
	const secure = "https://web.archive.org/"
	if strings.HasPrefix(u, insecure) {
		return secure + strings.TrimPrefix(u, insecure)
	}
	return u
}
...
func run(args []string) error {
	...
	verified, missing, unchanged, archived := 0, 0, 0, 0
	for _, repo := range candidates {
		...
		verified++
		j := joinHF(id, keys, aliases)
		// Enrich the LINKED repos only: an unlinked repo never enters the seed, so a
		// lookup for it would spend a polite request (and a second of cadence) on a row
		// that is discarded. Best-effort — a miss returns "" and is not an error, so the
		// harvest never fails, skips or reorders because the archive was unreachable.
		//
		// ORDER MATTERS: this runs AFTER verifyRepo has returned. hfDoer captures the
		// status of the most recent response, so the archive fetch overwrites the Hub
		// one; the 304/404 verdicts must already have been read (they have — verifyRepo
		// reads them before returning). Do not hoist this above the verify.
		if j.Linked {
			if snapshot := lookupArchivedURL(ctx, c, hubRepoBase+j.Repo); snapshot != "" {
				j.ArchivedURL = snapshot
				archived++
			}
		}
	...
	fmt.Fprintf(os.Stderr,
		"bestiary-hf: %d candidates fetched; %d verified (%d linked, %d unlinked), %d absent(404/401), %d unchanged(304), %d rate-limited(429), %d archive snapshots found; stamped huggingface ingested_at=%s\n",
		len(candidates), verified, len(out.Nomina), len(unlinked), missing, unchanged, d.count429, archived, snapshot)
	return nil
}

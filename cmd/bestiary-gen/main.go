...
func writeModelsdevUnlinked(models []bestiary.ModelInfo, meta []bestiary.EntityMetadata) error {
	...
	envelope := ModelsdevUnlinkedEnvelope{
		Comment: "Codegen-emitted models.dev join-disagreement report (DO NOT EDIT). Each id is a " +
			...
			"CAUTION: not every row here can be aliased cleanly. Some ids collapse under the current " +
			"decomposition onto a COARSE entity already shared by other lab models (e.g. " +
			"google/gemini-omni-flash-preview -> gemini/flash, poolside/laguna-xs-2.1 -> laguna, " +
			"openai/gpt-realtime-2.1 -> gpt) — aliasing those would OVERWRITE another row's metadata or " +
			"misattribute it to a whole family. Those await entity-granularity work (a tracked deferral, " +
			"not a curation gap); see the modelsdev_aliases.json _comment before adding an alias.",
		SchemaVersion: 1,
		Count:         len(ids),
		Unlinked:      ids,
	}

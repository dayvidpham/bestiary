...
func TestEntityConstants_Unique(t *testing.T) {
	...
	const wantEntityCount = 987 // 957 -> 940: the global free demotion retires 17 keys (0 added); the demoted instances re-home onto their surviving siblings. 940 -> 939: the qwen3-coder-next-fp8-1m suppress-pin extended to the unprefixed spelling retires qwen/coder@3#1m (1 key, 0 added) — its '1m' was a 1M-context tier marker, not a parameter size, so the InferX instance rejoins qwen/coder@3. 939 -> 947 with the ling/inkling/kling collision split: the upstream catalog labels all 14 Inkling + klingai rows raw_family "ling", so both product lines were folded onto inclusionAI's Ling family. Splitting them retires bare `ling` (it held only the 6 mislabelled Inkling instances, so it empties) and the phantom `kling-v2@6`, and adds `inkling`, `kling@2.6` and the 8 `kling/v*` video keys: -2 +10 = +8. 947 -> 946 with the keyspace-wide mimo normalization: the mimo series letter leaves the entity key (its family record now declares series_letter_in_key false) and the four speech/tier tokens are curated as a per-family series-tier extension, so the ten mimo keys collapse to nine — mimo, mimo/flash, mimo/pro, mimo/v2.5-tts, mimo/v2.5-tts-voiceclone, mimo/v2.5-tts-voicedesign, mimo/v@2.5, mimo/v@2.5{pro}, mimo/v@2{omni}, mimo/v@2{pro} retire and mimo@2.5, mimo@2.5{pro}, mimo@2.5{tts}, mimo@2.5{tts,voiceclone}, mimo@2.5{tts,voicedesign}, mimo@2{flash}, mimo@2{omni}, mimo@2{pro}, mimo@2{tts} take their place: -10 +9 = -1. All 93 instances are conserved; nine of the ten retirements are pure renames and the tenth (mimo/pro, 1 instance) merges onto mimo@2.5{pro}. 946 -> 945 with the cogito variant pin: the dash-glued togetherai spelling deepcogito/cogito-v2-1-671b had minted a phantom one-instance "Cogito v1" line, and pinning it to the (variant "v", version "2.1") pair its dotted siblings carry merges it onto the repaired dotted key — 1 key retired, 0 added. (The decomposition commit that precedes it renames cogito/v2.1-671b#671b to that key and moves no count; a later commit re-reads the glued 'v' as a version PREFIX rather than a variant, so the repaired key is spelled cogito@2.1#671b at the tip. That rename moves no count either, and measured against the release baseline it changes neither the retired set nor the added set: cogito/v@2.1#671b is absent from both sides of the cumulative diff.) 945 -> 942 with the gpt tier re-key: luna/sol/terra become VARIANTS of family `gpt`, so the 12 tier keys (gpt-luna, gpt-luna/pro, gpt-luna@5.6, gpt-luna/pro@5.6 and the sol/terra equivalents) retire and 9 take their place (gpt/<tier>, gpt/<tier>@5.6, gpt/<tier>@5.6{pro}). The undated {pro} pair per tier does NOT survive: with all six venice squashed-`56` ids pinned to 5.6, every -pro instance is dated, so gpt/<tier>{pro} has no occupant. -12 +9 = -3. 942 -> 933 with the redundant leading-token strip: an id that repeats, as its first token, a fact another axis already carries (the serving provider's own slug, or the lab the Creator axis attributes the family to) pushed the version scan one token late, so 165 records keyed an UNDATED sibling of the entity they belong to. With the prefix gone they land on their published version: 14 undated keys retire and 5 dated ones are minted (the other 9 merge onto dated keys that already existed). -14 +5 = -9. 933 -> 930 with the general bare-integer series-compound family recovery: a provider that reports a COMPOUND series family (raw_family "kimi-k2", "kimi-k3") kept that compound verbatim, because the version-pattern table matches only a DOTTED series number, so bare-integer compounds fell through to passthrough and stranded their models on compound-family keys of their own. Recovering the base family generally retires kimi-k2, kimi-k2{instruct} and kimi-k3 (their occupants merge onto the pre-existing kimi/k@2, kimi/k@2{instruct} and kimi/k@3) and retires kimi{instruct} as well (its sole occupant, a K2 Instruct row that sat on the bare kimi line with no version, re-homes onto kimi/k@2{instruct}); it adds exactly one key, kimi/coder, for the two umans rows whose id names no series token so the seam declines and the residual token becomes the variant. -4 +1 = -3. 930 -> 989 with the 2026-08-28 models.dev catalog refresh: providers 170 -> 204, model rows 5,765 -> 7,430. 87 keys retire and 146 are minted (930 - 87 + 146 = 989). Almost all of that is upstream churn; four curation pins ride along, each re-retiring a key the refresh had re-minted through a spelling the pipeline could not read: the inkling ':'-suffixed rows are pinned to family `inkling` (bare `ling` retires again), the xiaomi "-crof" backend-labelled rows to mimo@2.5{pro} (undated `mimo/pro` retires again), the four tier-before-version NVIDIA Nemotron 3.5 Lightning spellings to version 3.5 (undated `nemotron#30b-a3b` retires again), trendyol-asure-12b to family `asure` (gemma#12b retires, asure#12b is minted), vercel's eight new fish-audio rows to family `fish-audio` (they had all landed in the junk-bucket family "o", so `o` and `o/pro` retire and fish-audio/s@1, fish-audio/s@2{pro}, fish-audio/s@2.1{pro} and fish-audio/transcribe@1 are minted), kilo's openai/gpt-5.6-sol-discounted to (gpt, sol, 5.6) — "discounted" is a reseller pricing label, not identity, and unpinned it minted a bare gpt@5.6 that hijacked every gpt/5.6 lookup — and inferx's mimo-v25 to version 2.5 (the dot-lost "v25" spelling had minted a one-instance phantom mimo@25). Two keys the earlier curation had retired are legitimately LIVE again because upstream minted a real occupant for each: `gpt/pro` (edenai openai/gpt-pro-latest, a rolling undated alias) and `ministral#8b{instruct}` (pioneer mistralai/Ministral-8B-Instruct-2410). 989 -> 987 with the two round-2 review pins that land after the refresh. requesty's "inkling-256k" is pinned to the bare `inkling` family: a served context length is a serving fact, never a release — the requesty row carries the same 262144 window at the same 1.8700 / 4.6800 price as llmtr's bare thinkingmachines/inkling — so inkling@256k, the keyspace's only context-length-shaped version, is never minted. nano-gpt's "Gemma-4-31B-Claude-4.6-Opus-Reasoning-Distilled" is pinned to (gemma, 4): the 2026-08-28 catalog newly stamps raw_family "claude" on a Google Gemma 4 31B distill, which credited Anthropic with it on the flagship Opus line, so the row rejoins gemma@4#31b and claude/opus#31b is never minted. Both keys were among the 146 the refresh minted and neither existed at the 930-key baseline, so the refresh link is restated as 930 - 87 + 144 = 987. No key retires that did not already retire.
...
func TestEntityConstants_EntityByKeyRoundTrip(t *testing.T) {
	...
	// 11 -> 9 with the 2026-08-28 catalog refresh: groq and cloudflare-ai-gateway each
	// retired their Llama 4 Scout row upstream and the github-models provider left the
	// catalog entirely (-3 providers), while the new llmgateway-providers provider adds
	// one (+1). 11 - 3 + 1 = 9.
	if provs := bestiary.ProvidersOf(scout.Ref); len(provs) != 9 {
		t.Errorf("ProvidersOf(EntityByKey(scout).Ref) = %d providers; want 9", len(provs))
	}
}
...
// compiles, resolves to an entity, and ProvidersOf returns 9 distinct providers across
// its 12 instances — no provider-flavored entity constant remains).
func TestProvidersOf_ScoutCensus(t *testing.T) {
	...
	// 13 -> 12 with the 2026-08-28 catalog refresh: three pinned rows are gone
	// (groq "meta-llama/llama-4-scout-17b-16e-instruct", cloudflare-ai-gateway
	// "workers-ai/@cf/meta/llama-4-scout-17b-16e-instruct", and the whole github-models
	// provider) and two arrive from the new llmgateway-providers provider
	// ("aws-bedrock/…" and "novita/llama-4-scout-17b-instruct"). 13 - 3 + 2 = 12.
	if len(ent.Instances) != 12 {
		t.Errorf("scout instances = %d; want 12", len(ent.Instances))
	}

	provs := bestiary.ProvidersOf(ent.Ref)
	if len(provs) != 9 {
		t.Errorf("ProvidersOf(scout) = %d providers; want 9 (across 12 instances)", len(provs))
	}
